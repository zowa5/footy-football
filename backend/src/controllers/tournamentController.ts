...
export const startTournament = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Check permissions
    if (
      tournament.creator.toString() !== req.user!._id.toString() &&
      req.user!.role !== "admin"
    ) {
    ...
    await generateTournamentMatches(tournament._id);
...
export const completeTournament = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!tournament.participants.includes(winnerId)) {
      return res
        .status(400)
